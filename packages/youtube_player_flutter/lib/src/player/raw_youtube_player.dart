...
class _RawYoutubePlayerState extends State<RawYoutubePlayer>
  ...
  bool get _wasPlaying =>
      _cachedPlayerState == PlayerState.playing ||
      _cachedPlayerState == PlayerState.buffering;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      ...
      case AppLifecycleState.inactive:
        // Captured here: the OS may already have paused the media by `paused`.
        _cachedPlayerState ??= controller?.value.playerState;
        break;
      case AppLifecycleState.paused:
        _cachedPlayerState ??= controller?.value.playerState;
        if (controller?.flags.playInBackground ?? false) {
          _resumeInBackground();
        } else {
          controller?.pause();
        }
  ...
  // Runs in every frame, so YouTube's player never sees the page go hidden.
  static const _keepVisibleScript = '''
(function () {
  ...
  ['visibilitychange', 'webkitvisibilitychange'].forEach(function (name) {
...
''';
  ...
  UnmodifiableListView<UserScript>? get _userScripts {
    if (!controller!.flags.playInBackground) return null;
    return UnmodifiableListView([
      UserScript(
        source: _keepVisibleScript,
